 ...
 * dimension callouts are visible at all. Uses claude-sonnet-5, not Haiku —
 * direct side-by-side testing on a drawing with a hard rotated dimension
 * (small nearby fixture label easily confused for the real dimension line)
 * showed Haiku genuinely misreading it a meaningful fraction of the time no
 * matter how the prompt or a multi-call voting scheme was tuned, while
 * Sonnet read it correctly on nearly every single independent call and
 * even explained the fixture-label distinction unprompted. A single Sonnet
 * call is the fix — not more prompt engineering or ensembling around a
 * weaker model's mistakes.
 */
